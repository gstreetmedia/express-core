...
exports.checkBoxOrRadio = (attr) => {
	...
	let items = options.map(
		...
		return `
			...
			<div class="form-check form-switch ${css}">
	...
	value += "<div class='col'>" + items.slice(start, end).join("") + "</div>";
	start = end;
	end = start + end;
	value += "<div class='col'>" + items.slice(start, end).join("") + "</div>";
	start = end;
	...
	return `<div class="row px-2">${value}</div>`;
};

exports.switch = (attr) => {
	return `
	<div class="form-check form-switch pt-1 pr-2 pb-1">
...
exports.input = (attr, property) => {
	...
	switch (property.type) {
		...
		case "object" :
			...
			break;
		case "string" :
			placeholder = `Some Value ${property.format ? `[${property.format}]` : ''}`;
			break;
		case "array" :
...
exports.jsonEditor = (attr) => {
	...
	<textarea class="form-control json-editor"
		  ...
		  minlength="${attr.minlength || ""}"
		  data-type="object"
>${ beautify(attr.value, null, 2, 100) }</textarea>`
};
...
<textarea class="d-none" 
			name="${attr.name}" 
			${attr.required ? 'required' : ''} 
			data-type="text">${attr.value ? attr.value : ''}</textarea>
`
};
