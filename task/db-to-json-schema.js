			...
			function (item) {
				...
				if (!fs.existsSync(validationPath)) {
					...
					for (let key in item.properties) {
						...
						//s += "\t\tif(!data[key] && validator[" + key + "].default) { data[key] =  validator[" + key + "].default}\n\t\t\t";
						switch (item.properties[key].type) {
							case "string" :
								switch (item.properties[key].format) {
									case "uuid" :
										s += "return validator.isUUID(data[key]);";
										break;
									case "date" :
									case "date-time" :
										s += "if (typeof data[key] === 'string') { return true }\n\t\t\t";
										s += "return _.isDate(data[key]);";
										break;
									default :
										if (keyProxy === "email") {
											s += "return validator.isEmail(data[key]);";
										} else if (keyProxy.indexOf("url") !== -1) {
											s += "return validator.isURL(data[key]);";
										}  else if (keyProxy.indexOf("ipaddress") !== -1) {
											s += "return validator.isIP(data[key]);";
										} else {
											s += "return _.isString(data[key]);";
										}
							...
							case "number" :
								switch (item.properties[key].format) {
									case "integer" :
										s += "return _.isInteger(data[key]);";
										break;
									default :
										s += "return _.isNumber(data[key]);";
										break;
							...
							case "object" :
								switch (item.properties[key].format) {
									case "array" :
										s += "return _.isArray(data[key]);";
										break;
									default :
										s += "return _.isObject(data[key]);";
										break;

								}
								break;
							case "boolean" :
								s += "return _.isBoolean(data[key]);";
								break;
							default :
								s += "return data[key];";
						}
